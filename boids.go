...
var (
    arena = geom.CuboidCentred(160, 160, 160)

    boids struct {
        positions  geom.SliceVec3
        directions geom.SliceVec3
        colours    gfx.SliceColour
    }
...
func updateBoids() {
    ...
    for i, iPos := range boids.positions {
        ...
        var avoid, align, deltaAvg geom.Vec3
...
func spawnBoids() {
    boidsT.Append(geom.Vec3{}, geom.Vec3{1, 0, 0}, gfx.Red)

    for i := 0; i < boidsCount; i++ {
        pos := geom.Vec3Rand(arena)
        dir := geom.Vec3NormRand()
        boidsT.Append(pos, dir, gfx.ColourRand())
    }
}

func drawBoids(w *gfx.WinDraw, view geom.Mat4) {
    verts := [5]geom.Vec3{
        {0, 0, 3},
        {1, 1, -1},
        {-1, 1, -1},
